...
/**
 * Sorgente unica di criticità dei progetti.
 *
 ...
 * del team leader e il focus score personale. Le funzioni qui sotto sono pure
 * e vanno usate da tutte le viste.
 */
...
export const CRITICALITY_THRESHOLDS = {
  ...
  /** progresso oltre il quale un progetto è considerato "in chiusura" per stato di avanzamento */
 ...
 * punti sotto l'obiettivo, `warning` tra -10 e -5 punti.
 */
export function classifyMargin(
  residualMargin: number,
  targetMargin: number,
  budget: number,
): Severity | 'profit' | 'unknown' {
...
export function displayProgress(p: {
  billing_type?: string | null;
  start_date?: string | null;
  end_date?: string | null;
  progress?: number | null;
}, today: Date = new Date()): number | null {
...
export function projectedOverrunPct(
  margin: Pick<ProjectMarginRow, 'totalCost' | 'targetBudget'> | undefined,
  progress: number | null,
): number | null {
...
export function evaluateProjectCriticality(
  ...
  const marginClass =
    residual != null && margin
      ? classifyMargin(residual, targetMargin, margin.budget)
      : 'unknown';
  ...
  if (budgetPct != null && budgetLevel !== 'none') reasons.push(`budget ${pctLabel(budgetPct)}`);
...
export function groupProjectsByCriticality<T extends CriticalityProject>(
  ...
  const out = { at_risk: [], closing: [], in_progress: [], starting: [] } as Record<ProjectGroup, T[]>;
  ...
  for (const g of GROUP_ORDER) {
    out[g].sort((a, b) => {
      ...
      const rank = (severityRank[sb?.level ?? 'none'] ?? 0) - (severityRank[sa?.level ?? 'none'] ?? 0);
...
/**
 * Hook: unisce progetti e margini e restituisce i segnali di criticità condivisi.
 */
export function useProjectCriticality<T extends CriticalityProject>(projects: T[]) {
  ...
  return { signals, groups, margins: marginsMap ?? new Map<string, ProjectMarginRow>(), isLoading };
}
