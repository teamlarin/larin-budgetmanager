...
export const MemberDashboard = ({
  ...
  const hoursData = [{
    name: 'Oggi',
    pianificate: stats.todayPlannedHours,
    confermate: stats.todayConfirmedHours
  }, {
    name: 'Settimana',
    pianificate: stats.weekPlannedHours,
    confermate: stats.weekConfirmedHours
  }];
  ...
  return <div className="space-y-6">
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-5 gap-4">
        ...
        <Card variant="stats">
          ...
          <CardContent variant="stats">
            ...
            <div className="flex items-center gap-2 mt-1">
              <Progress value={stats.weeklyContractHours > 0 ? Math.min(stats.weekConfirmedHours / stats.weeklyContractHours * 100, 100) : 0} className="h-2 flex-1" />
      ...
      {weeklyCalendar && weeklyCalendar.length > 0 && <Card>
          <CardHeader className="flex flex-row items-center justify-between">
            <div>
              <CardTitle className="flex items-center gap-2">
                ...
                {weekDateRange && <span className="text-sm font-normal text-muted-foreground ml-2">
                    ({weekDateRange})
                  </span>}
              </CardTitle>
              <CardDescription>Panoramica delle tue attività pianificate</CardDescription>
            </div>
            {onWeekChange && <div className="flex items-center gap-2">
          ...
          <CardContent>
            ...
            return <div key={day.day + day.date} className={`p-3 rounded-lg border text-center transition-colors ${day.isToday ? 'border-primary bg-primary/5 ring-2 ring-primary/20' : 'border-border'} ${hasActivities ? 'hover:bg-muted/50 cursor-pointer' : ''}`} onClick={() => hasActivities && setSelectedDayForActivities(day)}>
                    ...
                    <div className="mt-2 space-y-1">
                      ...
                      {day.planned > 0 && <Progress value={Math.min(completionRate, 100)} className="h-1 mt-1" />}
                    </div>
                  </div>;
          })}
            </div>
        ...
        <DialogContent className="max-w-lg">
          ...
          <ScrollArea className="max-h-[400px]">
            <div className="space-y-2 pr-4">
              {selectedDayForActivities?.dayActivities && selectedDayForActivities.dayActivities.length > 0 ? selectedDayForActivities.dayActivities.map(activity => <div key={activity.id} className="flex items-center justify-between p-2 rounded-lg border gap-3">
                  ...
                  </div>) : <p className="text-sm text-muted-foreground text-center py-4">Nessuna attività per questa giornata</p>}
            </div>
          </ScrollArea>
