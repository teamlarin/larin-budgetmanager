...
export const PaymentModesManagement = () => {
  ...
  const createMutation = useMutation({
    mutationFn: async (data: Omit<PaymentMode, "id">) => {
      const { error } = await supabase.from("payment_modes").insert(data);
      if (error) throw error;
    },
    onSuccess: () => {
      queryClient.invalidateQueries({ queryKey: ["payment-modes-admin"] });
      queryClient.invalidateQueries({ queryKey: ["payment-modes"] });
      toast({ title: "Modalità di pagamento creata" });
      closeDialog();
    },
    onError: (error: Error) => {
  ...
  const updateMutation = useMutation({
    mutationFn: async ({ id, ...data }: PaymentMode) => {
      const { error } = await supabase.from("payment_modes").update(data).eq("id", id);
      if (error) throw error;
    },
    onSuccess: () => {
      queryClient.invalidateQueries({ queryKey: ["payment-modes-admin"] });
      queryClient.invalidateQueries({ queryKey: ["payment-modes"] });
      toast({ title: "Modalità di pagamento aggiornata" });
      closeDialog();
    },
    onError: (error: Error) => {
      toast({ title: "Errore", description: error.message, variant: "destructive" });
  ...
  const deleteMutation = useMutation({
    ...
    onSuccess: () => {
      queryClient.invalidateQueries({ queryKey: ["payment-modes-admin"] });
      queryClient.invalidateQueries({ queryKey: ["payment-modes"] });
      toast({ title: "Modalità di pagamento eliminata" });
    },
    onError: (error: Error) => {
      toast({ title: "Errore", description: error.message, variant: "destructive" });
    },
  ...
  const openCreateDialog = () => {
    const maxOrder = paymentModes.length > 0 
      ? Math.max(...paymentModes.map(t => t.display_order)) + 1 
      : 0;
  ...
  const handleSubmit = (e: React.FormEvent) => {
    ...
    const result = paymentModeSchema.safeParse(formData);
    if (!result.success) {
      toast({
    ...
    const validatedData: Omit<PaymentMode, "id"> = {
    ...
    if (editingMode) {
      updateMutation.mutate({ id: editingMode.id, ...validatedData });
    } else {
  ...
  const handleDelete = (mode: PaymentMode) => {
    if (confirm(`Eliminare la modalità di pagamento "${mode.label}"?`)) {
    ...
    <Card>
      ...
      <CardContent>
        <p className="text-sm text-muted-foreground mb-4">
          Definisci le tipologie di pagamento disponibili (es. Anticipo, Saldo, Acconto, Rata)
        </p>
        <Table>
          <TableHeader>
            <TableRow>
          ...
          <TableBody>
            {paymentModes.map((mode) => (
              <TableRow key={mode.id}>
                ...
                <TableCell className="font-mono text-sm">{mode.value}</TableCell>
                <TableCell>{mode.label}</TableCell>
                <TableCell>
                  <Switch
                    checked={mode.is_active}
                    onCheckedChange={(checked) => {
                      updateMutation.mutate({ ...mode, is_active: checked });
                    }}
                  />
                </TableCell>
                <TableCell>
                  <div className="flex gap-2">
                    <Button variant="ghost" size="icon" onClick={() => openEditDialog(mode)}>
                      <Pencil className="h-4 w-4" />
                    </Button>
                    <Button variant="ghost" size="icon" onClick={() => handleDelete(mode)}>
            ...
            {paymentModes.length === 0 && (
              <TableRow>
                <TableCell colSpan={5} className="text-center text-muted-foreground py-8">
                  Nessuna modalità di pagamento configurata
                </TableCell>
              </TableRow>
            )}
          </TableBody>
        ...
        <Dialog open={dialogOpen} onOpenChange={setDialogOpen}>
          <DialogContent>
            <DialogHeader>
              <DialogTitle>
                {editingMode ? "Modifica Modalità di Pagamento" : "Nuova Modalità di Pagamento"}
              </DialogTitle>
              <DialogDescription>
                {editingMode ? "Modifica i dati della modalità di pagamento" : "Inserisci i dati per la nuova modalità di pagamento"}
              </DialogDescription>
            </DialogHeader>
