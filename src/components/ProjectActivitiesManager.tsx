...
import { ExternalLink, X, Users, UserCheck, UserX, Plus, Trash2, Calendar, Folder, Pencil, Clock, FileDown, Check, Target, FileText, ChevronDown } from 'lucide-react';
...
export const ProjectActivitiesManager = ({
  ...
  const { data: userRates = {} } = useQuery<Record<string, number>>({
    ...
    queryFn: async () => {
      ...
      const profiles = await fetchHourlyRatesForCosting(assignedUserIds);
      const map: Record<string, number> = {};
  ...
  return <div className="space-y-6">
      ...
      {activities.length > 0 && (() => {
        ...
        const assignedActivities = activities.filter(a => getAssignedUsers(a.id).length > 0).length;
        ...
        <CardHeader className="flex flex-col gap-2">
          ...
          {canViewCosts && activities.length > 0 && (() => {
            ...
            const activitiesBudget = projectData?.manual_activities_budget != null
              ...
              : activities.reduce((sum, a) => sum + (a.total_cost || 0), 0);
