...
function ScheduledActivity({
  ...
  const activityContent = (
    ...
    }} className={`absolute left-1 right-1 shadow-sm border-l-4 overflow-hidden select-none ${isDragging ? 'cursor-grabbing z-50 opacity-80' : 'cursor-grab z-10'} ${categoryBorderColor} ${isCompleted ? 'bg-green-100 dark:bg-green-900/30' : isTrackingNow ? 'bg-blue-100 dark:bg-blue-900/30' : 'bg-card'}`} onClick={handleClick}>
      {/* Resize handle top */}
...
{index === 0 && dayTracking.map(tracking => <ScheduledActivity key={tracking.id} tracking={tracking} workDayStartHour={visibleHours[0]} onSaveResize={(id, start, end) => updateTrackingTimeMutation.mutate({
          ...
          <DragOverlay>
            {activeActivity && <div className="p-3 border bg-background shadow-lg opacity-90">
                <div className="flex flex-col gap-1">
            ...
            {activeScheduledTracking?.activity && <div className="p-3 border-l-4 border-primary bg-primary/10 shadow-lg opacity-90 min-w-[150px]">
                <div className="flex flex-col gap-1">
