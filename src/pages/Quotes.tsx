...
import { FileText, Trash2, Download, Search, ArrowUpDown, Edit, MoreVertical, Eye, ExternalLink } from 'lucide-react';
...
const Quotes = () => {
  ...
  const uniqueClients = useMemo(() => {
    const clients = allQuotes
      .filter(q => q.budgets?.clients?.name)
      .map(q => q.budgets!.clients!.name);
  ...
  const filteredAndSortedQuotes = useMemo(() => {
    ...
    if (searchTerm) {
      ...
      filtered = allQuotes.filter(quote => 
        ...
        quote.budgets?.name.toLowerCase().includes(term) || 
        quote.budgets?.clients?.name?.toLowerCase().includes(term)
      );
    }
  ...
  const handleDownloadPdf = async (quote: Quote) => {
    ...
    try {
      ...
      const { data: budget, error: budgetError } = await supabase.from('budgets').select(`*, clients (*)`).eq('id', budgetId).single();
      if (budgetError) throw budgetError;

      let account_profile = null;
      if (budget.account_user_id) {
        const { data: profileData } = await supabase.from('profiles').select('first_name, last_name').eq('id', budget.account_user_id).maybeSingle();
        account_profile = profileData;
      }

      const projectWithProfile = { ...budget, account_profile };

      const { data: budgetItems, error: itemsError } = await supabase.from('budget_items').select('*').or(`budget_id.eq.${budgetId},project_id.eq.${budgetId}`).eq('is_product', true).order('display_order');
      if (itemsError) throw itemsError;

      let services: any[] = [];
      if (budget.budget_template_id) {
        const { data: servicesData, error: servicesError } = await supabase.from('services').select('*').eq('budget_template_id', budget.budget_template_id);
  ...
  const handleDelete = async (quoteId: string) => {
    ...
    if (error) {
      toast({ title: 'Errore', description: 'Errore durante l\'eliminazione del preventivo.', variant: 'destructive' });
      return;
    }
    toast({ title: 'Preventivo eliminato', description: 'Il preventivo è stato eliminato con successo.' });
    refetch();
  };
                ...
                <Table>
                  <TableHeader>
                    <TableRow>
                      ...
                      {userRole !== 'team_leader' && <TableHead className="w-[100px]"></TableHead>}
                    </TableRow>
                  </TableHeader>
                  <TableBody>
                          ...
                          <TableCell>
                            <div className="flex items-center gap-1">
                              <Button
                                ...
                                disabled={downloadingQuote === quote.id || !hasPermission(userRole, 'canDownloadQuotes')}
                                title="Scarica PDF"
                              >
                              ...
                              <DropdownMenu>
                                ...
                                <DropdownMenuContent align="end" className="bg-background">
                                  ...
                                  {quote.budget_id && (
                                    <DropdownMenuItem onClick={() => navigate(`/budgets/${quote.budget_id}`)}>
