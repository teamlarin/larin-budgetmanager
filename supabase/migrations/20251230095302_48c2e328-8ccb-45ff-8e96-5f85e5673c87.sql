-- Fix delete_user_completely function - auth.refresh_tokens.user_id is varchar
...
BEGIN
  ...
  DELETE FROM public.budget_items WHERE project_id IN (
    SELECT id FROM public.projects WHERE user_id = _user_id::text
  );
  
  -- Update projects to remove references - account_user_id is UUID, user_id is text
  UPDATE public.projects SET account_user_id = NULL WHERE account_user_id = _user_id;
  UPDATE public.projects SET user_id = NULL WHERE user_id = _user_id::text;
